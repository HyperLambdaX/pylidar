...
- 交付形态：本机 `pip install --no-build-isolation -e .` 可装、`from pylidar.segmentation import ...` 可调
...
每个里程碑都是**独立可验证、独立 commit**。`pip install -e .` + `pytest -q` 必须全绿。每个里程碑完成后**停下汇报**，等用户决定再开下一个。
...
- 验证点（macOS）：`pip install --no-build-isolation -e .` 成功；`python -c "import pylidar; print(pylidar.__version__); import pylidar._core; print(pylidar._core.ping())"` 不报错
...
- 验证点：`pytest -q` 全绿；输出与 spec 内 fixture 在 `np.allclose(rtol=1e-6, atol=0)` 容差内一致
...
dev = ["pytest>=8", "pytest-benchmark>=4"]
...
```bash
uv sync                                  # 创建 .venv，装 dev deps
source .venv/bin/activate
pip install --no-build-isolation -e ".[dev]"
pytest -q
```

明确**不**用 `uv pip install -e .`（rd.md：uv 对 scikit-build-core editable rebuild 钩子支持弱）。
...
| writing-plans 出 plan | **每次只对一个里程碑出 plan**（先 M0），不一次性 plan 4 个 | 通过/打回 |
...
1. `pip install --no-build-isolation -e ".[dev]"` 成功，无平台特异编译错误
2. `python -c "import pylidar; from pylidar.segmentation import dalponte2016, silva2016, li2012, lmf_points, lmf_chm, chm_smooth, watershed"` 不报错
3. `pytest -q` 全绿
...
- Ubuntu 平台 `pip install -e .` + `pytest -q` 跑通
...
| nanobind | 2.4 | C++/Python 绑定（构建期） | BSD-3 |
